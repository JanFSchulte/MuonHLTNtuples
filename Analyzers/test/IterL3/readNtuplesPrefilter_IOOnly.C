...
void readNtuplesPrefilter_IOOnly(TString inputfilename="../../../Tools/muonNtuple_SingleMuon_iterL3_DataRunF.root", std::string effmeasured="Navigation"){
  ...
  TFile* outfile = TFile::Open(Form("%s_eff_pre.root", effmeasured.c_str()),"RECREATE");
  ...
  TH1F* n_passing_events        = new TH1F("h_n_passing_events"     , "n_passing_events",    1,  0,    1 );
  ...
  // 	  pass=false;
  // 	  if (matchMuonWithL3(ev->muons.at(jmu),ev->tkmuonsh)) pass = true; 
